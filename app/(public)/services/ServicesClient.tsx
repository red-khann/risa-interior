        ...
        <nav className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8 mb-20 pb-8 border-b border-zinc-200 relative z-[60]" aria-label="Service Filters">
          <div className="relative w-full md:w-72" ref={dropdownRef}>
            <button 
              ...
              className="w-full flex justify-between items-center py-2 text-[10px] uppercase tracking-[0.3em] font-black text-zinc-900 hover:text-[#B89B5E] transition-all outline-none"
            >
                ...
                {departments.map((dept) => (
                  <button
                    ...
                    className={`w-full text-left px-6 py-4 text-[9px] uppercase tracking-[0.2em] font-bold transition-colors border-b border-zinc-50 last:border-none ${
                      activeDept === dept ? "bg-zinc-50 text-[#B89B5E]" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900"
                    }`}
                  >
                    {dept}
                  </button>
          ...
          <div className="relative w-full md:w-64 group">
            ...
            <input
              ...
              className="bg-transparent border-b border-zinc-200 py-2 pl-8 text-[10px] tracking-widest outline-none focus:border-[#B89B5E] w-full uppercase font-black transition-all text-zinc-800"
              ...
              <article key={service.id}>
                <Link href={`/services/${service.slug}`} title={`View details for ${service.name}`}>
                  <div className="group relative p-10 md:p-12 bg-white border border-zinc-100 h-full flex flex-col justify-between transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.08)]">
                    <div className="absolute top-0 left-0 w-0 h-[2px] bg-[#B89B5E] group-hover:w-full transition-all duration-700 ease-in-out" aria-hidden="true" />
                    <div className="space-y-8">
                      <div className="flex justify-between items-start">
                        <span className="text-[9px] uppercase tracking-[0.4em] text-[#B89B5E] font-black">{service.service_type}</span>
                    ...
                    <div className="pt-10 mt-10 border-t border-zinc-50 flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-bold text-zinc-300 group-hover:text-[#B89B5E] transition-colors">{service.starting_price}</span>
                    </div>
                  </div>
                </Link>
              </article>
