...
export default function BlogClient({ initialPosts }: BlogClientProps) {
  ...
  const StarIcon = ({ fillPercentage }: { fillPercentage: number }) => {
    const gradientId = useMemo(() => `grad-blog-${Math.random().toString(36).substr(2, 9)}`, []);
    return (
  ...
  }, [searchQuery, activeCategory, sortOrder, initialPosts]);

  const BLOG_CATEGORIES = useMemo(() => {
    const types = Array.from(new Set(initialPosts.map(p => p.category).filter(Boolean)));
  ...
  return (
    <main className="pt-32 pb-20 bg-[var(--bg-warm)] min-h-screen selection:bg-[var(--accent-gold)]/10 overflow-x-hidden">
      <section className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <nav className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-20 pb-10 border-b border-zinc-200 relative z-[50]">
              ...
              <article key={post.id} className="group grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-20 items-center animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="md:col-span-5 space-y-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <Link href={`/blog/${post.slug}`}>
                      <h2 className="text-3xl lg:text-5xl font-bold leading-[1.0] text-[var(--text-primary)] group-hover:text-[var(--accent-gold)] transition-colors uppercase tracking-tighter">{post.title}</h2>
                    </Link>
                  </div>
                  
                  {post.excerpt?.trim() && (
                    <blockquote className="text-zinc-500 font-light text-lg leading-relaxed italic font-serif border-none p-0 bg-transparent shadow-none">"{post.excerpt}"</blockquote>
                  )}
                  
                  <Link href={`/blog/${post.slug}`}>
                    <button className="text-[10px] uppercase tracking-[0.5em] font-black text-[var(--accent-gold)] border-b-2 border-[var(--accent-gold)]/10 pb-1 pt-4 hover:border-[var(--accent-gold)] transition-all">Read Full Story —</button>
                  </Link>
                </div>
              </article>
            ))
