    ...
    <div className="min-h-screen bg-[#F7F5F2] pb-20 animate-in fade-in duration-700">
        ...
        <div className="max-w-[1440px] mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-8">
            <Link href="/admin/services" className="group flex items-center gap-3 transition-all">
              <div className="w-8 h-8 rounded-full border border-zinc-200 flex items-center justify-center group-hover:border-[#B89B5E]">
      ...
      <main className="max-w-[1440px] mx-auto p-12 grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <section className="bg-white p-12 border border-zinc-100 shadow-sm space-y-10">
            <div className="space-y-4">
              <label className="flex items-center gap-2 text-[9px] uppercase font-bold tracking-[0.4em] text-[#B89B5E]">
                <Target size={12} /> Service Designation
              </label>
              <input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} type="text" placeholder="SERVICE TITLE..." className="w-full bg-transparent border-b border-zinc-100 py-4 text-4xl font-light outline-none focus:border-[#B89B5E] text-zinc-800" />
            </div>
            <div className="space-y-4">
              <label className="text-[9px] uppercase font-bold tracking-[0.4em] text-zinc-400">Detailed Narrative</label>
              <textarea value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} rows={12} placeholder="Detail your methodology..." className="w-full bg-zinc-50/50 p-10 text-base leading-relaxed outline-none font-serif italic text-zinc-600 resize-none border-none" />
            </div>
          </section>
          ...
          <section className="bg-white p-8 border border-zinc-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              ...
              <div className="space-y-4">
                ...
                <textarea value={formData.heroAltText} onChange={(e) => setFormData({...formData, heroAltText: e.target.value})} rows={4} placeholder="Describe this image..." className="w-full p-4 bg-zinc-50 border-none text-[11px] italic text-zinc-500 outline-none resize-none focus:ring-1 focus:ring-[#B89B5E]/20" />
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white border border-zinc-100 p-8 shadow-sm space-y-8">
            <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  }} className="w-full p-4 bg-zinc-50 border-none text-[10px] font-bold uppercase tracking-widest outline-none appearance-none cursor-pointer">
                ...
                {(!PRESET_TYPES.includes(formData.serviceType) || formData.serviceType === "") && (
                  <input type="text" value={formData.serviceType} onChange={(e) => setFormData({...formData, serviceType: e.target.value})} placeholder="DEFINE CATEGORY..." className="w-full p-4 bg-zinc-50 border border-zinc-100 text-[10px] font-bold uppercase tracking-widest text-[#B89B5E] outline-none" />
                )}
            </div>
            ...
            <div className="space-y-4 pt-6 border-t border-zinc-50">
                ...
                {(!GEO_REACH_OPTIONS.includes(formData.availability) || formData.availability === "") && (
                  <input type="text" value={formData.availability} onChange={(e) => setFormData({...formData, availability: e.target.value})} placeholder="E.G. PAN INDIA" className="w-full p-4 mt-2 bg-zinc-50 border border-zinc-100 text-[10px] font-bold uppercase tracking-widest text-[#B89B5E] outline-none" />
                )}
            </div>
          ...
          <div className="bg-[#1C1C1C] p-8 text-white space-y-8 shadow-2xl border-t-4 border-[#B89B5E]">
            <header className="flex items-center justify-between border-b border-zinc-800 pb-4">
              <h3 className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#B89B5E] flex items-center gap-2">
                <Search size={14} /> Search Safeguard
              </h3>
              <div className={`w-2 h-2 rounded-full ${seoAudit.isReady ? 'bg-green-500' : 'bg-amber-500'}`} />
            </header>
            <div className="space-y-4">
              <input value={formData.focusKeyword} onChange={(e) => setFormData({...formData, focusKeyword: e.target.value})} placeholder="FOCUS KEYWORD" className="w-full bg-zinc-900 border-none p-4 text-[10px] font-bold tracking-widest text-[#B89B5E] outline-none" />
              <textarea value={formData.metaDescription} onChange={(e) => setFormData({...formData, metaDescription: e.target.value})} rows={3} placeholder="META DESCRIPTION" className="w-full bg-zinc-900 border-none p-4 text-[10px] font-bold tracking-widest text-zinc-400 outline-none resize-none" />
            </div>
          </div>
        </div>
      </main>
